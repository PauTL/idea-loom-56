...
const WorkflowStepper = ({ activeStep, onStepChange }: Props) => {
  return (
        ...
        <div key={step.label} className="flex items-center flex-1 last:flex-initial">
          ...
          >
            ...
            <span
              className={`text-xs font-medium whitespace-nowrap transition-colors ${
                i === activeStep ? "text-primary font-semibold" : "text-muted-foreground group-hover:text-foreground"
              }`}
            >
              {step.label}
            </span>
          ...
          {i < steps.length - 1 && (
            <div className="flex-1 mx-3 mt-[-18px]">
              <div className={`h-0.5 rounded-full ${i < activeStep ? "bg-step-active/40" : "bg-step-pending/30"}`} />
            </div>
          )}
        </div>
      ))}
